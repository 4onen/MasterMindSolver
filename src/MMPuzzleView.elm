...
viewPeg numColors disable i j mc =
    Html.td []
        [ Html.input
            ...
            , Html.Attributes.max <| String.fromInt numColors
...
viewReqs numPegs i heart d =
    Html.input
        ...
        , Html.Attributes.max (String.fromInt numPegs)
        , Html.Attributes.maxlength 1
        , Html.Attributes.value (String.fromInt d)
        , Html.Events.onInput
            (String.toInt
                >> Maybe.withDefault 0
                >> (if heart then
        ...
        , Html.Attributes.style "border-left" "4px solid black"
        ]
        []
...
viewSolutionTable model =
    let
        ...
        cnt =
            List.length filteredPossibilities
    in
    if cnt < 1 || cnt > 32 then
        Html.div []
            [ Html.text
                ("There are "
                    ++ (String.fromInt <| List.length filteredPossibilities)
                    ++ " possibilities."
                )
            ]
        ...
        filteredPossibilities
            |> List.map (List.map String.fromInt >> String.join ", " >> Html.text >> (\p -> Html.tr [] [ Html.td [] [ p ] ]))
            |> Html.table []
...
applyClue ( cs, h1, d ) guess =
    let
        zip =
            List.Extra.zip cs guess
        ...
        ( notHeartClues, notHeartGuess ) =
            List.unzip notHeartZip

        notEither =
            List.foldl findDot notHeartClues notHeartGuess

        numDots =
            List.length notHeartZip - List.length notEither
    in
    (List.Extra.count identity hs == h1) && numDots == d
